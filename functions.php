...
// SEO
add_action ( 'wp_head','wp_keywords' );
add_action ( 'wp_head','wp_description' );
...
function i_keywords() {
  ...
  foreach ( get_the_category ( $post->ID ) as $category )
    ...
    if(get_option("i_keywords")){
      $keywords = get_option("i_keywords");
    } else {
      $keywords = 'iFalse,iFalse主题,一款界面清新、轻松上手的WordPress主题';
    }
  } elseif (is_tag ()) { 
...
//   } elseif (is_search ()) {
//     $description = $blog_name . ": '" . esc_html ( $s,1 ) . "' 的搜索Y果";
//   } else {
//     $description = $blog_name . "有关 '" . trim ( wp_title ( '',false ) ) . "' 的文章";
//   }
//   $description = mb_substr ( $description,220,'utf-8' ) . '..';
